...
export default function WhatIfIAmScreen() {
  ...
  const convertToBase64 = async (uri: string): Promise<string> => {
    if (Platform.OS === 'web') {
      try {
        console.log('[WhatIfIAm] Converting web image:', uri.substring(0, 100));
        const response = await fetch(uri);
        if (!response.ok) {
          throw new Error(`Failed to fetch image: ${response.status}`);
        }
        let blob = await response.blob();
        console.log('[WhatIfIAm] Original blob size:', blob.size, 'bytes');
        
        // 压缩图片
        const maxWidth = 800;
        const quality = 0.7;
        blob = await compressImageWeb(blob, maxWidth, quality);
        console.log('[WhatIfIAm] After compression:', blob.size, 'bytes');
        ...
        return new Promise((resolve, reject) => {
          ...
          reader.onloadend = () => {
            ...
            console.log('[WhatIfIAm] Conversion complete, base64 size:', Math.round(base64Data.length / 1024), 'KB');
            resolve(base64Data);
          };
        ...
        console.error('[WhatIfIAm] Web conversion error:', error);
      ...
      try {
        ...
        console.error('[WhatIfIAm] Native compression error:', error);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 422) {
          throw new Error('该职业暂时无法处理，请更换照片或选择其他职业');
        }
