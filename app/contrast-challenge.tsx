...
export default function ContrastChallengeScreen() {
  ...
  const convertToBase64 = async (uri: string): Promise<string> => {
    if (Platform.OS === 'web') {
      try {
        console.log('[ContrastChallenge] Converting web image:', uri.substring(0, 100));
        const response = await fetch(uri);
        if (!response.ok) {
          throw new Error(`Failed to fetch image: ${response.status}`);
        }
        let blob = await response.blob();
        console.log('[ContrastChallenge] Original blob size:', blob.size, 'bytes');
        
        // 压缩图片
        const maxWidth = 800;
        const quality = 0.7;
        blob = await compressImageWeb(blob, maxWidth, quality);
        console.log('[ContrastChallenge] After compression:', blob.size, 'bytes');
        ...
        return new Promise((resolve, reject) => {
          ...
          reader.onloadend = () => {
            ...
            console.log('[ContrastChallenge] Conversion complete, base64 size:', Math.round(base64Data.length / 1024), 'KB');
            resolve(base64Data);
          };
        ...
        console.error('[ContrastChallenge] Web conversion error:', error);
      ...
      try {
        ...
        console.error('[ContrastChallenge] Native compression error:', error);
